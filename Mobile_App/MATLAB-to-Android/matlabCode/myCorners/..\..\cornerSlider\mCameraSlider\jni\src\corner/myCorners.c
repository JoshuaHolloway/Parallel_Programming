...
/*
 * function [R,G,B] = myCorners(R,G,B, fThresh, hThresh, mThresh)
 * Copyright 2014 - 2015 The MathWorks, Inc.
 * Arguments    : unsigned char R[101376]
 *                const unsigned char G[101376]
 *                const unsigned char B[101376]
 *                float fThresh
 *                float hThresh
 *                float mThresh
 * Return Type  : void
 */
void myCorners(unsigned char R[101376], const unsigned char G[101376], const
               unsigned char B[101376], float fThresh, float hThresh, float
               mThresh)
{
  int i0;
  (void)G;
  ...
  /* 'myCorners:4' R = R * 0.1; */
  for (i0 = 0; i0 < 101376; i0++) {
    R[i0] = (unsigned char)rt_roundd((double)R[i0] * 0.1);
  }
